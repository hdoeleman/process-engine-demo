...
@Component("logEnd")
public class LogEndHandler implements StepHandler {

    private static final Logger log = LoggerFactory.getLogger(LogEndHandler.class);

    private final SimulatedLatency latency;

    public LogEndHandler(SimulatedLatency latency) {
    ...
    public void execute(StepExecutionContext ctx) throws InterruptedException {
        ...
        log.info("[{}] activate-insurance completed successfully", ctx.getProcessInstanceId());
    }
}
